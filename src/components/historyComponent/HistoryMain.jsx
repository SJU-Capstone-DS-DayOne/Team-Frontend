...
import storeHistoryData from "../../clients/HistoryData";

export default function HistoryMain(prop) {
    const [reviewArray, setReviewArray] = useState([false, false, false]);
    const [textareaValue, setTextareaValue] = useState(["", "", ""]);
    const { setData } = useStore(storeHistoryData);
    ...
    const onClickReview = async (idx) => {
        const content = textareaValue[idx];
        const id =
            prop.prop.dateCourseRestaurantList[idx].dateCourseRestaurantId;
    ...
    return (
        <div className="flex flex-col px-6 pt-8 bg-white rounded-3xl min-w-[400px] max-w-[400px] w-[400px] flex-grow">
            <div className="w-full flex justify-between items-center pb-6 border-b-[1px]  border-b-[#d9d9d9] mb-6">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="text-[#c1c1c1] text-lg">
                        {
                            prop.prop.dateCourseRestaurantList[0].restaurant
                                .district
                        }
                    </div>
                </div>
            ...
            {prop.prop.dateCourseRestaurantList.map((val, idx) => (
                <div className="flex flex-col w-full mb-2" key={idx}>
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-4 p-4 rounded-2xl border-[1px] border-[#c1c1c1]">
                            {val.review ? (
                                <div className="flex items-center gap-3">
                                    <div className="text-sm text-[#c1c1c1]">{`${year}.${month}.${day}`}</div>
                    ...
                    {idx !== prop.prop.dateCourseRestaurantList.length - 1 ? (
                        <div className="pl-5 pb-2 border-r-[1px] bordeor-r-[#e2e2e2] w-1 h-8"></div>
                    ) : null}
                </div>
            ))}
        </div>
